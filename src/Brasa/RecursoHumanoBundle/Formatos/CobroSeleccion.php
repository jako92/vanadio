...
class CobroSeleccion extends \FPDF_FPDF {
    public static $em;
    ...
    public function Generar($em, $codigoCobro) {        
        ...
        $pdf = new CobroSeleccion();
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $pdf->SetFont('Times', '', 12);
        $this->Body($pdf);
        $pdf->Output("CobroSeleccion$codigoCobro.pdf", 'D');                
    } 
    ...
    public function Body($pdf) {
        ...
        $arSelecciones = self::$em->getRepository('BrasaRecursoHumanoBundle:RhuSeleccion')->findBy(array('codigoCobroFk' => self::$codigoCobro));
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 7);
        $var = 1;
        foreach ($arSelecciones as $arSeleccion) {                        
            $pdf->Cell(8, 6, $var, 1, 0, 'C');
            $pdf->Cell(22, 6, $arSeleccion->getCodigoSeleccionPk(), 1, 0, 'C');
            $pdf->Cell(30, 6, $arSeleccion->getNumeroIdentificacion(), 1, 0, 'C');
            $pdf->Cell(55, 6, utf8_decode($arSeleccion->getNombreCorto()), 1, 0, 'L');
            $pdf->Cell(20, 6, $arSeleccion->getFecha()->format("Y/m/d"), 1, 0, 'C');
            $pdf->Cell(35, 6, "SELECCION", 1, 0, 'L');
            $pdf->Cell(20, 6, number_format($arSeleccion->getVrServicio(),0, '.', ','), 1, 0, 'R');
            $pdf->Ln();
