...
class FormatoExamen extends \FPDF_FPDF {
    ...
    public function Header() {
        ...
        $arContenido = new \Brasa\GeneralBundle\Entity\GenContenidoFormatoSecundario();
        $arContenido = self::$em->getRepository('BrasaGeneralBundle:GenContenido')->find(6);
        ...
        $this->Cell(35, 6, "CODIGO: " . $arContenido->getCodigoFormatoIso(), 1, 0, 'L', 1);
        $this->SetXY(168, 24);
        $this->Cell(35, 6, utf8_decode("VERSIÓN: " . $arContenido->getVersion()), 1, 0, 'L', 1);
        $this->SetXY(168, 30);
        $fechaVersion = "";
        if ($arContenido->getFechaVersion()) {
            $fechaVersion = $arContenido->getFechaVersion()->format('Y-m-d');
        }
        $this->Cell(35, 6, utf8_decode("FECHA: " . $fechaVersion), 1, 0, 'L', 1);
        //
