...
class FormatoContratoAdicion extends \FPDF_FPDF {
    
    public static $em;
    
    public static $codigoContrato;
    
    public static $codigoContratoAdicion;
    
    public function Generar($em, $codigoContrato,$codigoContratoAdicion) {        
        ob_clean();
        //$em = $miThis->getDoctrine()->getManager();
        self::$em = $em;
        self::$codigoContrato = $codigoContrato;
        self::$codigoContratoAdicion = $codigoContratoAdicion;
        $pdf = new FormatoContratoAdicion();
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $pdf->SetFont('Times', '', 12);
        $this->Body($pdf);
        $pdf->Output("ContratoOtroSi$codigoContrato.pdf", 'D');        
