...
class CobroVisita extends \FPDF_FPDF {
    public static $em;
    ...
    public function Generar($em, $codigoCobro) {        
        ...
        $this->Body($pdf);
        $pdf->Output("CobroVisita$codigoCobro.pdf", 'D');                
    } 
    ...
    public function Body($pdf) {
        ...
        $arVisitas = self::$em->getRepository('BrasaRecursoHumanoBundle:RhuVisita')->findBy(array('codigoCobroFk' => self::$codigoCobro));
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 7);
        $var = 1;
        foreach ($arVisitas as $arVisita) {                        
            $pdf->Cell(8, 6, $var, 1, 0, 'C');
            $pdf->Cell(22, 6, $arVisita->getCodigoVisitaPk(), 1, 0, 'C');
            $pdf->Cell(30, 6, $arVisita->getEmpleadoRel()->getNumeroIdentificacion(), 1, 0, 'C');
            $pdf->Cell(55, 6, utf8_decode($arVisita->getEmpleadoRel()->getNombreCorto()), 1, 0, 'L');
            $pdf->Cell(20, 6, $arVisita->getFecha()->format("Y/m/d"), 1, 0, 'C');
            $pdf->Cell(35, 6, $arVisita->getVisitaTipoRel()->getNombre(), 1, 0, 'C');
            $pdf->Cell(20, 6, number_format($arVisita->getVrTotal(),0, '.', ','), 1, 0, 'R');
            $pdf->Ln();
