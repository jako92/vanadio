...
class CobroPoligrafia extends \FPDF_FPDF {
    public static $em;
    ...
    public function Generar($em, $codigoCobro) {        
        ...
        $pdf = new CobroPoligrafia();
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $pdf->SetFont('Times', '', 12);
        $this->Body($pdf);
        $pdf->Output("CobroPoligrafia$codigoCobro.pdf", 'D');                
    } 
    ...
    public function Body($pdf) {
        ...
        $arPoligrafias = self::$em->getRepository('BrasaRecursoHumanoBundle:RhuPoligrafia')->findBy(array('codigoCobroFk' => self::$codigoCobro));
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 7);
        $var = 1;
        foreach ($arPoligrafias as $arPoligrafia) {                        
            $pdf->Cell(8, 6, $var, 1, 0, 'C');
            $pdf->Cell(22, 6, $arPoligrafia->getCodigoPoligrafiaPk(), 1, 0, 'C');
