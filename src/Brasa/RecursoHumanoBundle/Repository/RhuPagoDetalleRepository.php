...
class RhuPagoDetalleRepository extends EntityRepository {
    ...
    public function prestacionalCertificadoIngreso($codigoEmpleado, $fechaDesde, $fechaHasta) {
        ...
        $dql = "SELECT SUM(pd.vrPago) as prestacional FROM BrasaRecursoHumanoBundle:RhuPagoDetalle pd JOIN pd.pagoRel p JOIN pd.pagoConceptoRel pc "
                . "WHERE p.estadoPagado = 1 AND pc.conceptoCesantia = 0 AND pc.prestacional= 1 AND p.codigoEmpleadoFk = " . $codigoEmpleado . " "
                . "AND p.fechaDesdePago >= '" . $fechaDesde . "' AND p.fechaDesdePago <= '" . $fechaHasta . "'";
        $query = $em->createQuery($dql);
        $arrayResultado = $query->getResult();
        $vrPagoPrestacional = $arrayResultado[0]['prestacional'];
        if ($vrPagoPrestacional == null) {
            $vrPagoPrestacional = 0;
        }
        return $vrPagoPrestacional;
    }
    ...
    public function noPrestacionalCertificadoIngreso($codigoEmpleado, $fechaDesde, $fechaHasta) {
        ...
        $query = $em->createQuery($dql);
        $arrayResultado = $query->getResult();
        $vrPagoNoPrestacional = $arrayResultado[0]['noPrestacional'];
        if ($vrPagoNoPrestacional == null) {
