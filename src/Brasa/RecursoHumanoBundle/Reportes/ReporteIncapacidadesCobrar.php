...
class ReporteIncapacidadesCobrar extends \FPDF_FPDF {
    public static $em;
    ...
    public function Generar($miThis, $em, $dql) {        
        ...
        self::$em = $em;
        self::$strDql = $dql;
        $pdf = new ReporteIncapacidadesCobrar('L');
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $pdf->SetFont('Times', '', 12);
        $this->Body($pdf);
        $pdf->Output("ReporteIncapacidadesCobrar.pdf", 'D');        
    } 
    ...
    public function Header() {
        ...
        $this->SetXY(10, 15);
        $this->Cell(283, 8, "LISTADO INCAPACIDADES POR COBRAR " , 1, 0, 'C', 1);
