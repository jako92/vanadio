...
class AspiranteController extends Controller {
    ...
    public function aplicarAction(Request $request, $codigoAspirante) {
        ...
        $form = $this->createFormBuilder()
                ->add('seleccionRequisicionRel', EntityType::class, array('class' => 'BrasaRecursoHumanoBundle:RhuSeleccionRequisito',
    ...
    /**
     * @Route("/rhu/movimientos/aspirante/bloquear/{codigoAspirante}", name="brs_rhu_movimiento_aspirante_bloquear")
     */
    public function bloquearAction(Request $request, $codigoAspirante) {

        $em = $this->getDoctrine()->getManager();
        $objMensaje = new \Brasa\GeneralBundle\MisClases\Mensajes();
        $arAspirante = new \Brasa\RecursoHumanoBundle\Entity\RhuAspirante();
        $arAspirante = $em->getRepository('BrasaRecursoHumanoBundle:RhuAspirante')->find($codigoAspirante);
        $permisoEspecial = $em->getRepository('BrasaSeguridadBundle:SegUsuarioPermisoEspecial')->permisoEspecial($this->getUser(), 119);
        $form = $this->createFormBuilder()
                ->add('comentarios', TextareaType::class, array('data' => $arAspirante->getComentarios(), 'required' => false))
                ->add('BtnBloquear', SubmitType::class, array('label' => 'Bloquear'))
        ...
        if ($form->isValid()) {
            if (!$permisoEspecial) {
                $objMensaje->Mensaje("error", "No tiene permisos para bloquear aspirantes, comuniquese con el administrador");
            } else {
                $arSeleccion = $em->getRepository('BrasaRecursoHumanoBundle:RhuSeleccion')->findBy(array('numeroIdentificacion' => $arAspirante->getNumeroIdentificacion()));
                if (count($arSeleccion) > 0) {
                    $objMensaje->Mensaje("error", "No puede bloquear un aspirante que se encuentra en proceso de seleccion");
                } else {
                    if ($form->get('BtnBloquear')->isClicked()) {
        ...
        return $this->render('BrasaRecursoHumanoBundle:Movimientos/Aspirante:bloquear.html.twig', array(
                    'arAspirante' => $arAspirante,
                    'form' => $form->createView()));
    }
