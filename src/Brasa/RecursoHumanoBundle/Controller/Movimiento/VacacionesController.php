...
class VacacionesController extends Controller {
    ...
    public function detalleAction(Request $request, $codigoVacacion) {
        ...
        if ($form->isValid()) {
            ...
            if ($form->get('BtnAnular')->isClicked()) {
                $strRespuesta = $em->getRepository('BrasaRecursoHumanoBundle:RhuVacacion')->anular($codigoVacacion);
                if ($strRespuesta != "") {
    ...
    private function listar() {
        ...
        $this->strSqlLista = $em->getRepository('BrasaRecursoHumanoBundle:RhuVacacion')->listaVacacionesDQL(
                $session->get('filtroCodigoCentroCosto'), $session->get('filtroIdentificacion'), $session->get('filtroPagado'), $session->get('filtroAutorizado')
        );
    }
    ...
    private function formularioDetalle($arVacacion) {
        ...
        if ($arVacacion->getEstadoAutorizado() == 1) {
            ...
            if ($arVacacion->getestadoAnulado() == 1) {
