...
class AcreditacionController extends Controller {
    ...
    private function generarExcel() {
        ...
        $em = $this->getDoctrine()->getManager();
        $dateFecha = new \DateTime('now');
        $strdateFecha = $dateFecha->format('Y-m-d');
        $session = new session;
        ...
        foreach ($arAcreditaciones as $arAcreditacion) {
            if ($arAcreditacion->getFechaVenceCurso() == "") {
                $estadoCurso = "NO TIENE";
            } else if ($strdateFecha >= $arAcreditacion->getFechaVenceCurso()->format('Y-m-d')) {
                $estadoCurso = "VENCIDO";
            } else {
                $estadoCurso = "VIGENTE";
            }
            if ($arAcreditacion->getEmpleadoRel()->getSubzonaRel()) {
                $subZonaEmpleado = $arAcreditacion->getEmpleadoRel()->getSubzonaRel()->getNombre();
            } else {
                $subZonaEmpleado = "";
            }
            if ($arAcreditacion->getEmpleadoRel()->getZonaRel()) {
                $zonaEmpleado = $arAcreditacion->getEmpleadoRel()->getZonaRel()->getNombre();
            } else {
                $zonaEmpleado = "";
            }
            if ($arAcreditacion->getEmpleadoRel()->getSubzonaRel()) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('S' . $i, $estadoCurso);
            if ($arAcreditacion->getFechaVenceCurso()) {
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $i, $arAcreditacion->getFechaVenceCurso()->Format('m-d-Y'));
            }
