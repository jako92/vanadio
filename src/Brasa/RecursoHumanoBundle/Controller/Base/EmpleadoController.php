...
class EmpleadoController extends Controller {
        ...
        if ($form->isValid()) {
            if ($form->get('BtnCargar')->isClicked()) {
                ...
                if ($objArchivo->getClientOriginalExtension() != NULL) {
                    if ($objArchivo->getClientOriginalExtension() == 'jpg') {
                        $arEmpleado = new \Brasa\RecursoHumanoBundle\Entity\RhuEmpleado();
                        $arEmpleado = $em->getRepository('BrasaRecursoHumanoBundle:RhuEmpleado')->find($codigoEmpleado);
                        $arConfiguracion = new \Brasa\GeneralBundle\Entity\GenConfiguracion();
                        $arConfiguracion = $em->getRepository('BrasaGeneralBundle:GenConfiguracion')->find(1);
    ...
    private function listar() {
        ...
        $this->strSqlLista = $em->getRepository('BrasaRecursoHumanoBundle:RhuEmpleado')->listaDQL(
                $session->get('filtroEmpleadoNombre'), $session->get('filtroCodigoCentroCosto'), $session->get('filtroEmpleadoActivo'), $session->get('filtroIdentificacion'), "", $session->get('filtroEmpleadoContratado'), $session->get('filtroCodigoEmpleado')
        );
    }
    ...
    private function generarExcel() {
        ...
        foreach ($arEmpleados as $arEmpleado) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('R' . $i, $arEmpleado->getFechaNacimiento()->Format('Y-m-d'))
    ...
    private function generarInterfaz() {
        ...
        foreach ($arEmpleados as $arEmpleado) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, "PERSONA NATURAL")                    
                    ->setCellValue('D' . $i, $arEmpleado->getNombreCorto())
                    ->setCellValue('E' . $i, $arEmpleado->getNombre1())
                    ->setCellValue('F' . $i, $arEmpleado->getNombre2())
                    ->setCellValue('G' . $i, $arEmpleado->getApellido1())
                    ->setCellValue('H' . $i, $arEmpleado->getApellido2())
                    ->setCellValue('I' . $i, $arEmpleado->getDireccion())
                    ->setCellValue('J' . $i, $arEmpleado->getCorreo())
                    ->setCellValue('K' . $i, $arEmpleado->getTelefono())
                    ->setCellValue('L' . $i, $arEmpleado->getCelular())
                    ->setCellValue('M' . $i, $arEmpleado->getFechaContrato()->format('Y/m/d'))
                    ->setCellValue('N' . $i, '0')
                    ->setCellValue('O' . $i, $arEmpleado->getCiudadRel()->getCodigoInterface())
                    ->setCellValue('P' . $i, '0')
                    ->setCellValue('Q' . $i, '')
                    ->setCellValue('R' . $i, 'N')
                    ->setCellValue('S' . $i, 'S')
                    ->setCellValue('T' . $i, 'S')
                    ->setCellValue('U' . $i, 'S')
                    ->setCellValue('V' . $i, $arEmpleado->getFechaNacimiento()->format('Y/m/d'));
            $i++;
        }
