...
class CartaLaboralController extends Controller {
        ...
        if ($form->isValid()) {
            if ($request->request->get('OpImprimir')) {
                ...
                if ($codigoCartaTipo == 0) {
                    ...
                    $objFormatoCarta->Generar($this, $em, $arUsuario, $usuarioCarta, 6, date('Y-m-d'), "", $codigoContrato, "", "", "", "", "", "", "");
                }
            ...
            if ($request->request->get('OpImprimirHistorico')) {
                $codigoContrato = $request->request->get('OpImprimirHistorico');
                $arContrato = new \Brasa\RecursoHumanoBundle\Entity\RhuContrato();
                $arContrato = $em->getRepository('BrasaRecursoHumanoBundle:RhuContrato')->find($codigoContrato);
                $objFormatoCartaHistorico = new \Brasa\RecursoHumanoBundle\Formatos\FormatoCartaHistorico();
                $objFormatoCartaHistorico->Generar($em, $arContrato->getCodigoEmpleadoFk());
            }
        }
    ...
    private function listar() {
        ...
        $this->strDqlLista = $em->getRepository('BrasaRecursoHumanoBundle:RhuContrato')->listaContratosCartaLaboralDQL(
                $session->get('filtroCodigoCentroCosto'), $session->get('filtroIdentificacion')
        );
    }
