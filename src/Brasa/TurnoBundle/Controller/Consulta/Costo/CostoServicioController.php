...
class CostoServicioController extends Controller {
    ...
    private function lista() {
        ...
        $this->strListaDql = $em->getRepository('BrasaTurnoBundle:TurCostoServicio')->listaDql(
                $session->get('filtroCodigoCliente'), $session->get('filtroTurAnio'), $session->get('filtroTurMes'), $session->get('filtroCodigoPuesto')
        );
    ...
    private function generarExcel() {
        ...
        foreach ($arCostoServicios as $arCostoServicio) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, $arCostoServicio->getCodigoPedidoDetalleFk())
                    ->setCellValue('D' . $i, $arCostoServicio->getClienteRel()->getNit())
                    ->setCellValue('E' . $i, $arCostoServicio->getClienteRel()->getNombreCorto())
                    ->setCellValue('F' . $i, $arCostoServicio->getPuestoRel()->getNombre())
                    ->setCellValue('G' . $i, $arCostoServicio->getConceptoServicioRel()->getNombre())
                    ->setCellValue('H' . $i, $arCostoServicio->getModalidadServicioRel()->getNombre())
                    ->setCellValue('I' . $i, $arCostoServicio->getPeriodoRel()->getNombre())
                    ->setCellValue('J' . $i, $arCostoServicio->getDiaDesde())
                    ->setCellValue('K' . $i, $arCostoServicio->getDiaHasta())
                    ->setCellValue('L' . $i, $arCostoServicio->getDias())
                    ->setCellValue('M' . $i, $arCostoServicio->getHoras())
                    ->setCellValue('N' . $i, $arCostoServicio->getHorasProgramadas())
                    ->setCellValue('O' . $i, $arCostoServicio->getCantidad())
                    ->setCellValue('P' . $i, $arCostoServicio->getVrCostoRecurso())
                    ->setCellValue('Q' . $i, $arCostoServicio->getVrTotal());
            $i++;
        }
