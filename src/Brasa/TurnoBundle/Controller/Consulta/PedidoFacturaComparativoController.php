...
class PedidoFacturaComparativoController extends Controller {
    ...
    public function listaAction(Request $request) {
        ...
        return $this->render('BrasaTurnoBundle:Consultas/Factura:pedidoFacturaComparativo.html.twig', array(
    ...
    private function lista() {
        ...
        $this->strListaDql = $em->getRepository('BrasaTurnoBundle:TurFacturaDetalle')->listaConsultaPedidoFacturaComparativoDql(
                $session->get('filtroTurnosCodigoFacturaTipo'), $session->get('filtroFacturaNumero'), $session->get('filtroCodigoCliente'), $strFechaDesde, $strFechaHasta, $session->get('filtroFacturaEstadoAnulado'));
    }
    ...
    private function generarExcel() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D1', 'SERVICIO PED')
                ->setCellValue('E1', 'FECHA PED')
                ->setCellValue('F1', 'NUM PED')
                ->setCellValue('G1', 'VR PED')
                ->setCellValue('H1', 'TIPO FACT')
                ->setCellValue('I1', 'SERVICIO FACT')
                ->setCellValue('J1', 'FECHA FACT')
                ->setCellValue('K1', 'NUM FACT')
                ->setCellValue('L1', 'VR FACT');

        $i = 2;
        $query = $em->createQuery($this->strListaDql);
        $arFacturasDetalles = new \Brasa\TurnoBundle\Entity\TurFacturaDetalle();
        $arFacturasDetalles = $query->getResult();

        foreach ($arFacturasDetalles as $arFacturaDetalle) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A' . $i, $arFacturaDetalle->getFacturaRel()->getClienteRel()->getNit())
                    ->setCellValue('B' . $i, $arFacturaDetalle->getFacturaRel()->getClienteRel()->getNombreCorto())
                    ->setCellValue('C' . $i, $arFacturaDetalle->getPedidoDetalleRel()->getPuestoRel()->getNombre())
                    ->setCellValue('D' . $i, $arFacturaDetalle->getPedidoDetalleRel()->getConceptoServicioRel()->getNombre())
                    ->setCellValue('E' . $i, $arFacturaDetalle->getPedidoDetalleRel()->getPedidoRel()->getFecha()->format('Y/m/d'))
                    ->setCellValue('F' . $i, $arFacturaDetalle->getPedidoDetalleRel()->getPedidoRel()->getNumero())
                    ->setCellValue('G' . $i, $arFacturaDetalle->getPedidoDetalleRel()->getVrTotalDetalle())
                    ->setCellValue('H' . $i, $arFacturaDetalle->getFacturaRel()->getFacturaTipoRel()->getNombre())
                    ->setCellValue('I' . $i, $arFacturaDetalle->getConceptoServicioRel()->getNombre())
                    ->setCellValue('J' . $i, $arFacturaDetalle->getFacturaRel()->getFecha()->format('Y/m/d'))
                    ->setCellValue('K' . $i, $arFacturaDetalle->getFacturaRel()->getNumero())
                    ->setCellValue('L' . $i, ($arFacturaDetalle->getTotal() * $arFacturaDetalle->getFacturaRel()->getFacturaTipoRel()->getOperacion()));
            $i++;
        }
        $objPHPExcel->getActiveSheet()->setTitle('PedidoFacturaComparativo');
        $objPHPExcel->setActiveSheetIndex(0);
        // Redirect output to a client’s web browser (Excel2007)
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="PedidoFacturaComparativo.xlsx"');
