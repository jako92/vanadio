...
class TurPedidoRepository extends EntityRepository {
    ...
    public function listaPendienteContabilizarCierreMesDql($numeroPedido = "", $codigoCliente = "", $strFechaDesde = "", $strFechaHasta = "") {
        $dql = "SELECT p FROM BrasaTurnoBundle:TurPedido p WHERE p.estadoContabilizado = 0 AND p.numero > 0 AND p.estadoCierreMes = 1";
        if ($numeroPedido != "") {
            $dql .= " AND p.numero = " . $numeroPedido;
        }
        if ($codigoCliente != "") {
    ...
    public function contabilizar($arrSeleccionados) {
        ...
        if (count($arrSeleccionados) > 0) {
            foreach ($arrSeleccionados AS $codigo) {
                $arPedido = new \Brasa\TurnoBundle\Entity\TurPedido();
                $arPedido = $em->getRepository('BrasaTurnoBundle:TurPedido')->find($codigo);
                if ($arPedido->getEstadoAutorizado() == 1 && $arPedido->getEstadoContabilizado() == 0 && $arPedido->getNumero() != 0) {
                    //Validar si el tercero existe para crearlo de no existir en la tabla ctbTercero
                    $arTercero = $em->getRepository('BrasaContabilidadBundle:CtbTercero')->findOneBy(array('numeroIdentificacion' => $arPedido->getClienteRel()->getNit()));
                    if (count($arTercero) <= 0) {
                        $arTercero = new \Brasa\ContabilidadBundle\Entity\CtbTercero();
                        $arTercero->setCiudadRel($arPedido->getClienteRel()->getCiudadRel());
                        $arTercero->setTipoIdentificacionRel($arPedido->getClienteRel()->getTipoIdentificacionRel());
                        $arTercero->setNumeroIdentificacion($arPedido->getClienteRel()->getNit());
                        $arTercero->setDigitoVerificacion($arPedido->getClienteRel()->getDigitoVerificacion());
                        $arTercero->setNombreCorto($arPedido->getClienteRel()->getNombreCorto());
                        $arTercero->setNombre1($arPedido->getClienteRel()->getNombre1());
                        $arTercero->setNombre2($arPedido->getClienteRel()->getNombre2());
                        $arTercero->setApellido1($arPedido->getClienteRel()->getApellido1());
                        $arTercero->setApellido2($arPedido->getClienteRel()->getApellido2());
                        $arTercero->setDireccion($arPedido->getClienteRel()->getDireccion());
                        $arTercero->setTelefono($arPedido->getClienteRel()->getTelefono());
                        $arTercero->setCelular($arPedido->getClienteRel()->getCelular());
                        $arTercero->setEmail($arPedido->getClienteRel()->getEmail());
                        $em->persist($arTercero);
                    }
                    $arComprobanteContable = $em->getRepository('BrasaContabilidadBundle:CtbComprobante')->find($arPedido->getPedidoTipoRel()->getCodigoComprobanteFk());
                    $codigoCuentaCartera = $arPedido->getPedidoTipoRel()->getCodigoCuentaCarteraFk();
                    $codigoCuentaIva = $arPedido->getPedidoTipoRel()->getCodigoCuentaIvaFk();
                    $codigoCuentaIngreso = $arPedido->getPedidoTipoRel()->getCodigoCuentaIngresoFk();
                    ...
                    if ($codigoCuentaCartera) {
                        ...
                        if ($arCuenta) {
                            ...
                            $arRegistro->setNumero($arPedido->getNumero());
                            $arRegistro->setNumeroReferencia($arPedido->getNumero());
                            $arRegistro->setFecha($arPedido->getFecha());
                            if ($arPedido->getPedidoTipoRel()->getTipoCuentaCartera() == 1) {
                                $arRegistro->setDebito($arPedido->getVrTotal());
                            } else {
                                $arRegistro->setCredito($arPedido->getVrTotal());
                            }
                        ...
                        $respuesta = "La cuenta cartera cliente en pedido tipo esta vacia";
                        break;
                    }
                    ...
                    if ($codigoCuentaIva) {
                        if ($arPedido->getVrIva() > 0) {
                            ...
                            if ($arCuenta) {
                                ...
                                $arRegistro->setNumeroReferencia($arPedido->getNumero());
                                $arRegistro->setFecha($arPedido->getFecha());
                                $arRegistro->setBase($arPedido->getVrBaseAiu());
                                if ($arPedido->getPedidoTipoRel()->getTipoCuentaIva() == 1) {
                                    $arRegistro->setDebito($arPedido->getVrIva());
                                } else {
                                    $arRegistro->setCredito($arPedido->getVrIva());
                                }
                        ...
                        $respuesta = "La cuenta iva en pedido tipo esta vacia";
                        break;
                    }

                    //Cuenta ingreso
                    if ($codigoCuentaIngreso) {
                        if ($arPedido->getVrSubtotal() > 0) {
                            $arCuenta = $em->getRepository('BrasaContabilidadBundle:CtbCuenta')->find($codigoCuentaIngreso);
                            if ($arCuenta) {
                                $vrTotalIngreso = $arPedido->getVrTotalServicio() + $arPedido->getVrTotalOtros();
                                $arRegistro = new \Brasa\ContabilidadBundle\Entity\CtbRegistro();
                                $arRegistro->setComprobanteRel($arComprobanteContable);
                                $arRegistro->setCuentaRel($arCuenta);
                                $arRegistro->setTerceroRel($arTercero);
                                ...
                                $arRegistro->setNumeroReferencia($arPedido->getNumero());
                                $arRegistro->setFecha($arPedido->getFecha());
                                if ($arPedido->getPedidoTipoRel()->getTipoCuentaIngreso() == 1) {
                                    $arRegistro->setDebito($vrTotalIngreso);
                                } else {
                                    $arRegistro->setCredito($vrTotalIngreso);
                                }
                        ...
                        $respuesta = "La cuenta ingreso en pedido tipo esta vacia";
                        break;
                    }

                    $arPedido->setEstadoContabilizado(1);
                    $em->persist($arPedido);
                }
            }
        }
