...
class Factura4 extends \FPDF_FPDF {
    ...
    public function Body($pdf) {
        ...
        foreach ($arFacturaDetalles as $arFacturaDetalle) {
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(15, 7, $arFacturaDetalle->getCodigoFacturaDetallePk(), 0, 0, 'L');
            $pdf->Cell(125, 7, $strDetalle, 0, 0, 'L');
            //$pdf->MultiCell(125, 3.5, $strDetalle, 1, 'L');
            $pdf->Cell(10, 7, $arFacturaDetalle->getPedidoDetalleRel()->getPedidoRel()->getNumero(), 0, 0, 'L');
            $pdf->Cell(15, 7, number_format($arFacturaDetalle->getCantidad(), 0, '.', ','), 0, 0, 'R');
            $pdf->Cell(30, 7, number_format($arFacturaDetalle->getSubtotal(), 0, '.', ','), 0, 0, 'R');
            $pdf->Ln();
    ...
    public function GenerarEncabezadoFactura($em) {
        ...
        $this->Image('imagenes/logos/logo.jpg', 13, 12, 43, 26);
        $this->ln(11);
        $this->SetFont('Arial', 'B', 12);
        $this->ln(5);
        $this->SetFont('Arial', 'B', 10);
        $this->Text(21, 40, "NIT " . $arConfiguracion->getNitEmpresa() . "-" . $arConfiguracion->getDigitoVerificacionEmpresa());
        //informacion empresa
        $this->SetFont('Arial','B',8);
        $this->SetXY(57, 7);
